...
// import { useSelector, useDispatch } from 'react-redux';  used for redux
...
const CourseDetailedPage = () => {
  ...
  const newSocket = io.connect('https://pureathma.online/');
  ...
  // DATA PASSED AND RECEIVED BY USEPARAMS METHOD
  // const { selectedCourse } = useParams();
  // const courseDetail = JSON.parse(decodeURIComponent(selectedCourse));
  ...
  const fullPayment = async (amount, courseId) => {
    userInterceptor({
      ...
      console.error('Error in place-order request:', err);
  ...
  function razorpayPayment(order) {
    var options = {
      "key": "rzp_test_jrrHKb6Ek3iFiJ",
      "amount": order.amount,
      "currency": "INR",
      "name": "Pure athma Booking",
      "description": "Test Transaction",
      "image": "/images/user/razorpay.png",
      "order_id": order.id,
  ...
  const verifyPayment = async (payment, order) => {
    userInterceptor({
      url: '/api/user/verify-payment',
      method: 'post',
      data: { payment: payment, order: order }
    }).then((response) => {
      if (response.data.success) {
        newSocket.emit('notification_user', { count: response.data.countUser, room: response.data.roomUser })
        newSocket.emit('notification_trainer', { count: response.data.countTrainer, room: response.data.roomTrainer })
        navigate('/profile')
      } else {
  ...
  return (
    <div className='course-detailed-page'>
      <HomeNavbar />
      <div className="course-detailed-page-course flex w-100 justify-center items-center h-screen mb-0  shadow-md rounded-xl" >
        {/* Left side (Image) */}
        <div className="mr-8 mt-4 rounded-xl">
          <img
            src={courseDetail.image}
            alt="Course"
            className="w-45 mt-5 h-50 object-cover rounded-xl"
          />
        </div>
        ...
        <div>
          {/* Course Category */}
          <div className="mb-4">
            <span className="text-lg ">{courseDetail.category}</span>
          </div>

          {/* Course Name */}
          <div className="mb-4">
            <span className="text-lg font-bold">Course:    {courseDetail.course_name}</span>
          </div>

          <div className="mb-4">
            <span className="text-lg ">{courseDetail.description}</span>
          </div>

          {/* Trainer Name and Image */}
          <div className="mb-4 flex items-center">
            {/* <img
              src="trainer-image.jpg"
              alt="Trainer"
              className="w-6 h-6 object-cover rounded-full mr-2"
            /> */}
            <span>By: {courseDetail.trainer_name}</span>
          </div>

          {/* Amount */}
          <div className="mb-4">
            <span className="text-lg">₹ {courseDetail.amount}</span>
          </div>

          {/* Purchase Button */}
          <div>
            <button onClick={() => fullPayment(courseDetail.amount, courseDetail._id)} className="px-4 py-2 course-detailed-button text-white rounded cursor-pointer">
              Purchase
            </button>
          </div>
        </div>
      ...
      <div class="w-8/12 mx-auto mt-0">
        {rating && < div className="mt-6" >
          ...
          {rating?.average === 2 && < div class="flex items-center mb-1 ">
            <svg class="w-4 h-4 text-yellow-300 mr-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
              <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z" />
            </svg>
            <svg class="w-4 h-4 text-yellow-300 mr-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
          ...
          {rating?.average === 3 && < div class="flex items-center mb-1 ">
            <svg class="w-4 h-4 text-yellow-300 mr-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
              <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z" />
            </svg>
            <svg class="w-4 h-4 text-yellow-300 mr-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
          ...
          {rating?.average === 4 && < div class="flex items-center mt-1 ">
            <svg class="w-4 h-4 text-yellow-300 mr-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
              <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z" />
            </svg>
            <svg class="w-4 h-4 text-yellow-300 mr-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
          ...
          {/* ------------average rating area end-------------------------- */}

          <div className="bg-gray-50 rounded-lg shadow-md p-4 mt-3">


            <div className="mt-4 items-center reviews_in_artist_view_page">
                  ...
                  <div className="flex items-center mb-4 space-x-4 ">
                    <div key={index} className="flex items-center mb-4 space-x-4 mr-20">
                      <article className='mt-4 mb-4'>
                        ...
                        {items?.starRating === 5 && < div class="flex items-center mb-1">
                          <svg class="w-4 h-4 text-yellow-300 mr-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
                            <path d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z" />
                          </svg>
                          <svg class="w-4 h-4 text-yellow-300 mr-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
